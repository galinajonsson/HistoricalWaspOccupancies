...
```{r plot-functions, message=FALSE, echo=FALSE}
#First I need to load the plotting functions I've written
############################ One Year ############################

plot.occDet.wasp1yr <- function(x, y = NULL, main = x$SPP_NAME, reg_agg = '', ...){
...
plot.occDet.wasp5yrs <- function(x, y = NULL, main = x$SPP_NAME, reg_agg = '', ...){
  ...
  spp_data$X <- row.names(spp_data)
  new_data <- spp_data[grepl(paste0("^psi.fs", reg_agg, "\\["),spp_data$X),]
  new_data$year <- (Year = (x$min_year +1894) + (as.numeric(gsub(paste0("psi.fs", reg_agg), "", gsub("\\[|\\]","", row.names(new_data)))))*5)
...
plot.occDet.wasp10yrs <- function(x, y = NULL, main = x$SPP_NAME, reg_agg = '', ...){
  ...
  spp_data$X <- row.names(spp_data)
  new_data <- spp_data[grepl(paste0("^psi.fs", reg_agg, "\\["),spp_data$X),]
  new_data$year <- (Year = (x$min_year +1889) + (as.numeric(gsub(paste0("psi.fs", reg_agg), "", gsub("\\[|\\]","", row.names(new_data)))))*10)
...
Here I will format the data and run the 12 models with varying spatial (1, 2, 5 and 10 km grid cells) and temporal (1, 5 or 10 year TPs) resolutions. Please note that I will describe each step in detail for model 1 and omitt any repeated descriptions in the formatting of subsequent models. 
